            ...
            {booking.start_location && (
              <div className="flex items-center gap-1.5 text-zinc-500 text-xs truncate">
                ...
                <span>
                  {booking.start_location}
                  {(() => {
                     try { return JSON.parse(booking.details).dep_terminal ? ` (T${JSON.parse(booking.details).dep_terminal})` : ''; }
                     catch(e) { return booking.details?.dep_terminal ? ` (T${booking.details.dep_terminal})` : ''; }
                  })()}
                </span>
                {booking.end_location && (
                  <>
                    ...
                    <span>
                      {booking.end_location}
                      {(() => {
                         try { return JSON.parse(booking.details).arr_terminal ? ` (T${JSON.parse(booking.details).arr_terminal})` : ''; }
                         catch(e) { return booking.details?.arr_terminal ? ` (T${booking.details.arr_terminal})` : ''; }
                      })()}
                    </span>
                  </>
                )}
...
// Transportation Card Component for Itinerary Tab
...
function SubItemRow({ sub, itineraryImageUrl, isLast, hasMore }: { sub: any; itineraryImageUrl: string | null; isLast: boolean; hasMore: boolean }) {
  ...
  const handleToggle = (e: React.MouseEvent) => {
    ...
    if (newShowNotes && rowRef.current) setTimeout(() => rowRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
  };
  ...
  return (
    <div ref={rowRef} className={clsx(
      ...
      itineraryImageUrl ? "text-white/90 bg-black/40 border-white/10" : "text-zinc-400 bg-zinc-900/50 border-zinc-800/50"
    )}>
      ...
      <div className="flex items-center justify-between gap-3 pl-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {sub.address && (
            <a 
              href={sub.address.startsWith('http') ? sub.address : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(sub.address)}`} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
              className={clsx("p-1.5 rounded-full transition-colors", itineraryImageUrl ? "text-white/70 hover:text-white hover:bg-white/10" : "text-zinc-500 hover:text-orange-500 hover:bg-zinc-800")}
            ><Map size={14} /></a>
          )}
          {(sub.notes || (sub.tags && sub.tags.length > 0)) && (
            <button onClick={handleToggle} className={clsx("p-1.5 rounded-full transition-colors", itineraryImageUrl ? "text-white/70 hover:text-white hover:bg-white/10" : "text-zinc-500 hover:text-white hover:bg-zinc-800")}>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden pl-2">
            {sub.tags && Array.isArray(sub.tags) && sub.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2 mb-1">
                {sub.tags.map((tag: string, idx: number) => <span key={idx} className={clsx("text-[10px] px-1.5 py-0.5 rounded border", itineraryImageUrl ? "text-white/80 border-white/20 bg-white/5" : "text-zinc-400 border-zinc-700 bg-zinc-800")}>{tag}</span>)}
              </div>
            )}
...
function ItineraryCard({ 
  ...
  useEffect(() => {
    if (hasExpandableContent) setIsExpanded(!isPastItem);
    else setIsExpanded(false);
  }, [isPastItem, hasExpandableContent]);

  const handleHeaderClick = () => {
    if (canEdit) onEdit();
    else if (hasExpandableContent) setIsExpanded(!isExpanded);
  };
                ...
                >
                  {item.next_transport_mode ? (
                    <>
                      ...
                      {(item.next_transport_time || item.next_transport_auto_time) && (
                        <span className="text-[9px] font-mono font-bold leading-none mt-0.5">
                          {item.next_transport_time ? item.next_transport_time.replace(' min', 'm') : (item.next_transport_auto_time || 'Auto')}
                        </span>
                      )}
                    </>
                  ) : <Plus size={18} />}
        ...
        <div className="absolute inset-0 p-5 flex flex-col z-10">
                  ...
                  {subItems.length > 0 && (
                    <div className="space-y-3">
                      {subItems.map((sub: any, idx: number) => (
                        <SubItemRow key={sub.id || idx} sub={sub} itineraryImageUrl={itineraryImageUrl} isLast={idx === subItems.length - 1} hasMore={idx < subItems.length - 1} />
                      ))}
                    </div>
                  )}
                </div>
          ...
          {(hasNotes || hasSubItems) && !canEdit && (
            <div className="flex justify-center mt-auto pt-2 shrink-0">
              <div 
                className={clsx("flex items-center justify-center w-10 h-6 rounded-full border backdrop-blur-md transition-colors duration-300 cursor-pointer hover:bg-white/10", itineraryImageUrl ? "bg-black/40 border-white/20 text-white" : "bg-zinc-800/80 border-zinc-700/50 text-zinc-400 hover:text-white")}
                onClick={(e) => { e.stopPropagation(); setShowDetails(!showDetails); }}
              >
                {showDetails ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
              </div>
    ...
    <div className="flex flex-col h-screen bg-black overflow-hidden overscroll-none">
      <div className="shrink-0 z-30 bg-black relative shadow-xl">
        <div className="relative h-64 w-full overflow-hidden">
          ...
          {user && (
            <button 
              onClick={() => { if (hasEditPermission) setIsEditMode(!isEditMode); else alert('您沒有編輯此行程的權限。'); }}
                  ...
                  return (
                    <div key={`itinerary-${item.id}`} className="space-y-2">
                      <ItineraryCard 
                        ...
                        onEdit={() => {
                          if (booking) { setEditingBooking(booking); setIsBookingFormOpen(true); }
                          else { setEditingItinerary(item); setIsItineraryFormOpen(true); }
                        }}
                ...
                Object.entries(
                  ...
                  <div key={category} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {categoryBookings
                        .sort((a, b) => parseISO(`${a.start_date}T${a.start_time}`).getTime() - parseISO(`${b.start_date}T${b.start_time}`).getTime())
                        .map(booking => (
                          <BookingCard
        ...
        {activeTab === 'settings' && hasEditPermission && (
          <div className="space-y-6">
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 shadow-lg">
              ...
              <TripSettingsForm 
                trip={trip} 
                onSuccess={() => { apiFetch(`/api/trips/${id}`).then(res => res.json() as Promise<Trip>).then(data => db.trips.put(data)); }} 
              />
            </div>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="relative w-full max-w-md z-10">
              <ItineraryForm 
                ...
                onSuccess={() => { setIsItineraryFormOpen(false); setEditingItinerary(null); apiFetch(`/api/trips/${id}/itineraries`).then(res => res.json() as Promise<Itinerary[]>).then(data => db.itineraries.bulkPut(data)); }} 
      ...
      <ConfirmDialog isOpen={confirmConfig.isOpen} title={confirmConfig.title} message={confirmConfig.message} confirmText={confirmConfig.confirmText} onConfirm={confirmConfig.onConfirm} onCancel={() => setConfirmConfig({ ...confirmConfig, isOpen: false })} />
    </div>
  );
}