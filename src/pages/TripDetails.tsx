...
import { Map, Info, Wallet, ArrowLeft, Calendar, Settings, Edit3, ChevronsUpDown, ChevronsDownUp, Unlock, Plus, DollarSign, Loader2 } from 'lucide-react';
...
export function TripDetails() {
  ...
  const isMember = user && (
    ...
    trip?.members?.some(m => Number(m.user_id) === Number(user.id))
  );
  ...
  const conflictedIdsInView = useMemo(() => {
    ...
    for (let i = 0; i < filteredItineraries.length - 1; i++) {
      ...
      if (curr.start_time && curr.end_time && next.start_time && next.end_time) {
        ...
        if (currTimes.end > nextTimes.start) {
  ...
  return (
    <div className="flex flex-col h-screen bg-black overflow-hidden overscroll-none">
      ...
      {/* 💡 修正 2 & 3：沉浸式 Hero Banner 佈局 */}
      <div className="shrink-0 z-30 relative shadow-xl w-full h-[240px] sm:h-[280px]">
        {/* 背景底圖 (Z = 0) */}
        <img src={tripCoverImageUrl} alt={trip.title} className="absolute inset-0 w-full h-full object-cover z-0" referrerPolicy="no-referrer" />
        
        {/* 黑色漸層遮罩 (Z = 10) */}
        <div className="absolute inset-0 bg-gradient-to-b from-black/80 via-black/20 to-black/90 z-10 pointer-events-none" />

        {/* 頂部 Header Bar (Z = 20) */}
        <div className="absolute top-0 left-0 right-0 z-20 px-4 flex items-center justify-between" style={{ paddingTop: 'max(1rem, env(safe-area-inset-top))' }}>
          <button onClick={() => navigate('/')} className="p-2.5 bg-black/20 backdrop-blur-md rounded-full text-white hover:bg-black/40 transition-colors border border-white/10 shadow-lg" >
            <ArrowLeft size={20} />
          </button>
          
          <div className="flex items-center gap-2">
            {hasEditPermission && (
              <button onClick={handleToggleEditMode} className={clsx( "p-2.5 backdrop-blur-md rounded-full transition-all border border-white/10 shadow-lg", isEditMode ? "bg-orange-500 text-white border-orange-500" : "bg-black/20 text-white hover:bg-black/40" )} >
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 px-5 pb-5">
          <div className="flex items-end justify-between gap-4">
            {/* 左側標題：flex-1 + truncate 讓文字超過時變成刪節號 */}
            <h1 className="text-2xl md:text-3xl font-black text-white truncate flex-1 leading-tight tracking-tight drop-shadow-md" title={trip.title}>
              {trip.title}
            </h1>

            {/* 右側日期：shrink-0 確保它絕對不被壓縮變形 */}
            <div className="shrink-0 flex flex-col items-end text-right">
              <span className="text-[9px] font-black text-orange-400 uppercase tracking-[0.2em] mb-1.5 drop-shadow-sm">
                Travel Dates
              </span>
              <div className="bg-white/10 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-white/20 shadow-lg flex items-center gap-1.5">
                <Calendar size={12} className="text-orange-400" />
                <span className="text-[11px] font-black text-white tracking-tight">
                  {validTripStartDate ? format(validTripStartDate, 'MM/dd') : ''} 
                  <span className="text-zinc-400 mx-1">→</span> 
                  {validTripEndDate ? format(validTripEndDate, 'MM/dd') : ''}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              {filteredItineraries.length > 0 ? (
                filteredItineraries.map((item, index) => {
                  ...
                  // const booking = (item.type === 'ACCOMMODATION' || item.type === 'RENTAL') && item.related_id 
          ...
          <div className="space-y-4">
            ...
            {filteredExpenses.length > 0 ? (
              filteredExpenses.map(expense => (
                <div key={expense.id} onClick={() => { if (canEdit) { setEditingExpense(expense); setIsFinanceFormOpen(true); } }} className={`bg-zinc-900 border border-zinc-800 rounded-3xl p-5 shadow-lg flex items-center justify-between transition-colors ${canEdit ? 'cursor-pointer hover:bg-zinc-800/50' : ''}`} >
